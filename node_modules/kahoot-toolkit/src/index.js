...
class Kahoot {
    ...
    async flood(amount, persistent) {
        ...
        for (let i = 0; i < amount; i++) {
            ...
            else await api.join(this.pin, this.name+' '+`${Math.random()}`.split(".")[1].slice(0,15-`${this.name}`.length+1));
        }
    }
    ...
    async autoplay(quizResolvable, debug, fully_autoplay) {
        ...
        if (debug) console.log(`${colors.bold("[").cyan}${colors.bold("Kahoot").magenta} ${colors.white("Client").white}${colors.bold("]").cyan}: Joining Game...`);
        const client = new api();

        client.on("Joined", () => {
            if (debug) console.log(`${colors.bold("[").cyan}${colors.bold("Kahoot").magenta} ${colors.white("Client").white}${colors.bold("]").cyan}: Successfully Joined Game as '${this.name}'!`)
        });

        client.on("QuizStart", () => {
            if (debug) console.log(`${colors.bold("[").cyan}${colors.bold("Kahoot").magenta} ${colors.white("Client").white}${colors.bold("]").cyan}: Quiz Started!`)
        });

        client.on("QuizEnd", (data) => {
            if (debug) console.log(`${colors.bold("[").cyan}${colors.bold("Kahoot").magenta} ${colors.white("Client").white}${colors.bold("]").cyan}: Quiz Ended! Finished with Rank ${data.rank} and ${data.totalScore} Points.`)
        });
        ...
        client.on("QuestionStart", async question => {
            ...
            if (debug) console.log(`${colors.bold("[").cyan}${colors.bold("Kahoot").magenta} ${colors.white("Client").white}${colors.bold("]").cyan}: Answering Question ${question.gameBlockIndex + 1} with ${answers.length === 1 ? `'${questionData.choices[0].answer}'` : `'${answerNames.join("' and '")}'` }...`);
            

            if (!fully_autoplay){
                let p = readline.question(`\n${colors.bold("do you want to get the question right? (y/n)").white} ${colors.bold(" >").magenta} `)
                console.log("")
                if(p.toLocaleLowerCase()=="y"){
                    question.answer(answers);
                } else if (p.toLocaleLowerCase()=="n"){
                    if(answers[0]-1 >= 1){
                        answers[0] = answers[0]-1
                        question.answer(answers);
                    } else if (answers[0]+1 <= 4){
                        answers[0] = answers[0]+1
                        question.answer(answers);
                    }
                }
        ...
        client.on("QuestionEnd", question => {
            if (debug) console.log(`${colors.bold("[").cyan}${colors.bold("Kahoot").magenta} ${colors.white("Client").white}${colors.bold("]").cyan}: Question Ended! ${question.isCorrect ? `(${"Correct".green}, +${question.points} Points)` : `(${"Incorrect".red})`} Currently Rank ${question.rank}`);
        })
